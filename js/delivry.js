$(document).ready(function () {
	// UTMIFY GLOBAL RECOVERY
	// Checks if there are any pending orders that failed to send
	(async function checkPendingUtmify() {

		const pendingStr = localStorage.getItem('utmify_pending_queue');
		...
		try {
			...
			if (ids.length > 0) {
				console.log(`[Utmify Recovery] Found ${ids.length} pending orders.`);
			}
			...
			for (const id of ids) {
				...
				try {
					...
					if (response.ok) {
						console.log(`[Utmify Recovery] Order ${id} recovered and sent!`);
						delete pending[id];
						localStorage.setItem('utmify_pending_queue', JSON.stringify(pending));
					}
				} catch (e) {
					console.error(`[Utmify Recovery] Failed to recover ${id}:`, e);
				}
			}
		} catch (e) {
			console.error("[Utmify Recovery] Error parsing queue", e);
		}
	})();
	...
	(async function checkTransactionStatus() {
		...
		if (localStorage.getItem(`processed_tx_${transactionId}`)) return;

		console.log(`[Status Check] Verifying transaction ${transactionId}...`);
		...
		try {
			...
			if (data.status === 'paid') {
				console.log("[Status Check] Transaction found as PAID! Triggering recovery...");

				// Mark as processed immediately to prevent loops if logic fails
				localStorage.setItem(`processed_tx_${transactionId}`, 'true');

				// Construct Utmify Payload locally
				const customer = JSON.parse(localStorage.getItem(`customer_data_${transactionId}`) || '{}');
				const items = JSON.parse(localStorage.getItem(`items_data_${transactionId}`) || '[]');

				// If data missing, we can't report.
				if (!customer.email || items.length === 0) {
					console.warn("[Status Check] Missing customer/item data, cannot report.");
					return;
				}

				let totalCents = 0;
				items.forEach(item => totalCents += (item.unitPrice * item.quantity));
				...
				const payload = {
					...
					customer: {
						...
						document: customer.document.number || customer.document || "00000000000",
						ip: "127.0.0.1"
					},
					...
					trackingParameters: {
						// Attempt to recover UTMs from storage if we saved them, or empty
						utm_source: null
					},
					commission: {
				...
				// Add to Pending Queue -> The existing Global Recovery will pick it up on next tick/reload
				// OR send immediately. Let's send immediately for speed.
						...
						if (obj.semEstoque) {
							...
							if (obj.rastreamento) {
								if (obj.pedido) {
									...
									$('#pedido .containerFinalizar #obrigado h3').html("Seu pedido foi recebido com sucesso.");
